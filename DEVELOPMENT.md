...
- **外部 CLI Agent 参考调研**：`CLI-AGENT-REFERENCE-RESEARCH.md` 已系统比较 `dsh-subagent-claude-code-wrapper`、`dsh-agent-conductor`、`dsh-cursor-acp`、`dsh-llm-agy`；`CLI-AGENT-FRAMEWORK-RESEARCH.md` 继续比较 `dsh-plugin-subagents`、`dsh-routed-subagent`、`dsh-plugin-product-subagents`、`dsh-custom-subagents`、`dsh-dag-orchestrator` 及官方/AlataStudio Claude Provider。后续将 CLI 工具升级为真正的可持续子代理时，以两份文档记录的 Driver、Provider、readiness、continuable、程序化 dispatch service、会话恢复和权限状态机结论为依据。
