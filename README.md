...
3. **能在 DSH 里用**：DSH 模型通过 `cli_dispatch` / `cli_<cli>_direct` / `cli_<cli>_subagent` 调用这些 CLI 干活，把结果回传进对话；每个 CLI 可独立配置权限能力（读文件 / 写文件 / 执行命令，exec 已承载联网意图）与审批模式（询问 / 自动拒绝），勾选的能力运行时静默放行、未勾选的触发时弹窗或自动拒绝，旧的只读 / 工作区可写 / 完全三档预设会自动映射为对应的能力组合；
...
cd plugin && npm run test:live  # 真实环境端到端验证（需已配置 Provider + 凭据）
