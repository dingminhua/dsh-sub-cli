...
1. **统一目录**：把各 Agent CLI 的二进制集中放到一个用户可选的目录（Windows 默认 `C:\dsh-clis`，macOS 默认 `~/dsh-clis/`），避免散落系统 PATH；用户修改目录后迁移插件托管内容。
...
以下以 macOS 默认目录为例；Windows 使用同样的内部结构，只把根目录换为 `C:\dsh-clis`。
