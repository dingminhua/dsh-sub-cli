...
- **Claude Code 排除 WebSearch/WebFetch**：实测发现 Claude Code 在 stream-json 模式下确实暴露 `WebSearch` 工具（system/init 事件 `tools` 数组），但 stream-json 协议不带审批事件、DSH 拿不到授权，结果是"工具请求权限时未获授权"。`claude-stream-json.js` 在每次 spawn 都加 `--disallowed-tools WebSearch,WebFetch`，让 LLM 在规划阶段就知道这俩不可用，避免 mid-turn 失败。Qwen/Codex 内部根本没有 web 工具（tools 数组不包含 web_*），不需要类似处理。**网搜/抓 URL 一律用 DSH 自带的 `advanced_search` / `web_fetch` / `platform_search`。
...
- 单元测试新增：Claude/Qwen driver 13 个（argv 组合、沙箱映射、session-id 提取、followup --resume、error 转换、多段文本拼接）；session-tools 3 个（全部 12 工具注册、followup/status/sessions/interrupt 调用路由）；subagent-tools 3 个（Claude/Qwen session-mode dispatch、session-mode 拒绝后台模式）；Claude driver `--disallowed-tools` 1 个（验证 WebSearch/WebFetch 被启动时排除）。
- 能力矩阵：README.md 加一节"各 CLI 工具能力矩阵"，明确哪些能力有、哪些没有，以及为何排除。
