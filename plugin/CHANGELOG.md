...
- **权限收敛为三能力（read / write / exec）**：删除「联网」开关——exec 已承载联网意图（npm install / git pull 属普通命令执行），存量 `network:true` 由 normalizer 映射到 exec。设置卡权限区改为单一三档下拉（只读 ⊆ 可写 ⊆ 可调用工具，三档 read 恒 true——读取默认放行、无运行时弹窗）；approval（询问 / 自动拒绝）只作用于档位未授予的能力。`capabilityKey` 映射 `command→exec`、`file-change→write`、`permissions(escalation)→exec`。
- **driver 层统一权限拦截（三 CLI 体验一致）**：三个 CLI 一律最高档启动（Claude `--permission-mode bypassPermissions`、Qwen `tools.approvalMode: yolo`、Codex app-server），由 driver 解析 stream-json `tool_use` / app-server protocol 事件，按 profile 检查 capability，统一走 `onPermissionRequest → resolvePermission() → ctx.approval.request()` GUI 弹窗。勾选=静默放行；未勾选=弹窗（ask）或自动拒绝（never）。`permissions.js` 新增 `CLAUDE_APPROVAL_METHODS` / `QWEN_APPROVAL_METHODS` 与 `normalizePermissionRequest(cli, method, params, context)` 统一 normalizer（Bash/NpmcliLifecyclePlugin→command、Write/MultiWrite/Edit/Delete→file-change、WebSearch/WebFetcher→exec、只读工具→null 静默放行）。Claude/Qwen 的 stream-json 单向无 ack，拒绝以 `finish(false)` 终止 turn；异步权限决策经 `Promise.resolve().then()` 挂起。
...
- **分工定型：主控调研，CLI 执行**：新增 `lib/capability-gate.js` 并对**三个 CLI 对称**拒绝联网调研任务（含 Claude Code）——话术为"联网调研由主控直接执行（DSH 自带 advanced_search 等），如需 CLI 处理材料请主控先调研再派发"，接入 `cli_<cli>_direct` / `cli_<cli>_subagent` / `cli_dispatch` 三条路径。删除"联网用 Claude Code"整条故事及其载体：`webTools` 标记（registry + client 双份）、按 Claude 放行的门禁分支、UI 对 Codex/Qwen 隐藏联网开关并**强制写回 `network:false`** 的行为。理由：Claude 的 WebSearch 是 Anthropic 的 server-side 工具，chat 型中转实测不执行（13 次调用 0 真实结果），不能作为依赖路径。设置卡的「联网」开关保留并改为**纯进程沙箱旋钮**（勾选 → danger-full-access，进程可联网，npm install / git clone 不受影响），三个 CLI 对称显示——顺带修复 UI 暗中把 Codex 沙箱从"完全"降档的隐性 bug。
