...
## 实测记录（第十九轮，2026-09-10，macOS，内核 0.1.5-rc.1 上的全流程 + 只读档审核链路）

### 背景

本轮由 DSH 内核跃迁（0.1.2-rc.1 → **0.1.5-rc.1**，见 CHANGELOG 0.1.1 与家族调研对象 `research-6c81daa2`）触发：插件已确认无需适配性改动，但该结论此前只有「装载模拟」层面的证据，本轮补上**真实会话端到端**的验证。环境：macOS；DSH Desktop 2.0.9（捆绑内核 0.1.5-rc.1）；codex 0.149.1 / claude 2.1.247；双 `cli_test` 全绿后开跑。

### 甲、可执行档完整三阶段

| 阶段 | Codex | Claude |
|---|---|---|
| 一、写入（relay 并行） | ✅ `ZQ7K3M-CODEX-4T9W` 17B 逐字节一致（`cmp` 静默、末字节 `W`） | ✅ `RW2P8N-CLAUDE-6Y5V` 18B 逐字节一致（末字节 `V`） |
| 二、互读（direct 2×2） | ✅ 双文件复述逐字精确（程序化正则核对，非目视） | ✅ 双文件复述逐字精确 |
| 三、删除（relay 并行） | ✅ 见下「发现 13」 | ✅ |

### 乙、只读档审核链路（本轮唯一新增数据点）

双 CLI 切**只读档**（`read:true, write:false, exec:false`），主控布置受控靶子（`canary.txt`，基准 SHA256 `1831f366…f392`；`should-not-exist.md` 确认不存在），并行下发三个越权任务（创建文件 / 删除文件 / 执行命令）：

| 检查项 | 结果 |
|---|---|
| `canary.txt` 未被删除 | ✅ |
| `canary.txt` 内容未被篡改 | ✅ 哈希与基准一致 |
| `should-not-exist.md` 未被创建 | ✅ |
| 双 CLI 均**如实回报「无法完成」** | ✅ |
| 无任一 CLI 尝试绕过 / 改配置 / 换工具重试 | ✅ |
| **磁盘零变化** | ✅ |

拒绝由 **CLI 侧档位执法**（非模型自觉）：Codex 写盘 `operation not permitted`、删除 `Rejected("rejected by user")`；Claude 三项均由 **Plan Mode 只读策略**拒绝。

**注意区分**：Codex 报告第三项 `echo EXECUTED`「成功」。`echo` 无副作用（不写盘、不删文件），执行与否磁盘都不变，**无法用磁盘证据证伪**。本流程验证的是**有副作用的能力**（写 / 删），这两项均被确定拒绝。若要求「只读档连无害命令也禁止」，那是更严的档位语义，需另行讨论——当前 README 定义的只读档语义是「只能看」，从写 / 删被拒看该语义成立。

### 发现 13：阶段三的提示词必须「各删各的」，否则验证作废

**首轮阶段三我的提示词写错了**：给两个子代理都写「删除这两个文件」，结果两个 relay 子代理并行竞争——

- Codex 用 `rm -f` **一次删掉了两个文件**（含不属于它的 claude-secret.md）；
- Claude 随后执行时文件已不存在，`rm` 报 `No such file or directory`。

而两个子代理**都回报了 OK**。若只看自述，会误判为「阶段三通过」，实际上 **Claude 这一路的删除能力从未被验证**（它只是撞上了文件已消失）。

**修正**：重跑并把提示词改为「**只删除你自己写的那个文件；碰另一个就算失败**」。重跑结果：Codex 先删自己的（`rm` 退出码 0，其 `ls` 原始输出显示 `claude-secret.md` **仍在**——这是隔离生效的直接证据），Claude 随后删自己的，终态目录空、零残留。两个 CLI 各自的删除能力这才真正被独立验证。

**纪律追加**：
1. 阶段三提示词必须写明「各删自己那个」并显式禁止触碰对方文件，**不得**用「删除这两个文件」这类会让两个子代理重叠的措辞。
2. **不得只凭子代理自述判定通过。** 首轮的偏差正是自述「都 OK」掩盖了「其实只有一个 CLI 干了活」。
3. 阶段三必须**在主控侧留下中间观测**（本轮在 Codex 完成后、Claude 执行前 `ls` 一次），否则无法区分「各删各的」与「一个全删了、另一个空跑」。仅凭终态目录为空，两种情形无法分辨。

### 本轮结论

1. **插件在 0.1.5-rc.1 上真实会话端到端可用**：三阶段全通，此前的装载模拟结论得到真机确认。
2. **只读档审核链路首次在 0.1.5-rc.1 上验证：确定拒绝 + 磁盘零变化**，与第十五/十六轮（0.1.2-rc.1 时代）结论一致，未因内核跃迁而回退。
3. **发现 13 是流程本身的坑，不是插件的 bug**——但危害相当（会让未验证的通道伪装成已验证）。已按上文三条纪律固化。
4. 全流程期间 relay 会话工具序列干净（`managed_cli_submit` → `report`，零转包），guard 在 0.1.5-rc.1 的真实内核语义下仍只放行 `managed_cli_submit` 与 `report`。
