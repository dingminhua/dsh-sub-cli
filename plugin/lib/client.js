...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    function RouteSelects(props) {
      ...
      var permission = props.permission || DEFAULT_PERMISSION;
      var group = null;
      ...
      return React.createElement("div", { className: "dsc-route" },
        ...
        React.createElement("label", { className: "dsc-field" }, t("row.permission"),
          React.createElement("select", { className: "dsc-select dsc-perm", value: permission, onChange: function (e) { props.onPermissionChange(e.target.value); } },
            PERMISSIONS.map(function (p) { return React.createElement("option", { key: p.id, value: p.id }, p.label); })
          )
        )
      );
    }
    ...
    function SetupRow(props) {
      ...
      return React.createElement("section", { className: "dsc-card" },
        ...
        CLIS.map(function (cli) {
          ...
          var fpOk = function (r) { return r && r.provider === (route.provider || null) && r.model === (route.model || null) && (r.reasoningEffort || null) === (route.reasoningEffort || null); };
          ...
          var guideText = "";
          if (f) {
            guideText = fillCli(t("row.guideFailed"));
          } else if (v) {
    ...
    function CliJobsView(props) {
      var jobs = props.useSessions(function (state) { return (state.jobsBySession && state.jobsBySession[props.sessionId]) || []; });
      var cliJobs = jobs.filter(function (job) { return /^cli-(codex|claude|qwen)$/.test(job.kind || ""); });
      var nowState = React.useState(Date.now());
      React.useEffect(function () {
        if (!cliJobs.some(function (job) { return job.status === "running" || job.status === "stopping"; })) return;
        var timer = setInterval(function () { nowState[1](Date.now()); }, 1000);
        return function () { clearInterval(timer); };
      }, [cliJobs.length, cliJobs.map(function (job) { return job.status; }).join("|")]);
      function duration(job) {
        var end = job.finishedAt || nowState[0];
        var seconds = Math.max(0, Math.floor((end - job.startedAt) / 1000));
        if (seconds < 60) return seconds + " 秒";
        return Math.floor(seconds / 60) + " 分 " + (seconds % 60) + " 秒";
      }
      if (!cliJobs.length) return React.createElement("div", { className: "dsc-jobs-empty" }, "当前会话还没有 CLI 后台任务。调用 cli_codex、cli_claude_code 或 cli_qwen 时传 run_in_background:true 即可在这里查看。");
      return React.createElement("div", { className: "dsc-jobs-view" },
        React.createElement("div", { className: "dsc-jobs-heading" }, "CLI 后台任务"),
        cliJobs.slice().sort(function (a, b) { return b.startedAt - a.startedAt; }).map(function (job) {
          var cli = (job.kind || "").replace(/^cli-/, "");
          var live = job.status === "running" || job.status === "stopping";
          return React.createElement("div", { className: "dsc-job-card", key: job.id },
            React.createElement("div", { className: "dsc-job-head" },
              React.createElement("span", { className: "dsc-job-cli" }, cli),
              React.createElement("strong", null, job.label),
              React.createElement("span", { className: "dsc-job-status " + (live ? "dsc-job-live" : "") }, job.status)
            ),
            React.createElement("div", { className: "dsc-job-meta" }, job.id + " · " + duration(job) + (job.detail ? " · " + job.detail : "")),
            React.createElement("div", { className: "dsc-job-hint" }, live ? "任务正在运行。模型可用 job_output 增量读取输出，也可用 job_kill 取消。" : "任务已结束；可让模型调用 job_output 读取最终输出。")
          );
        })
      );
    }
    ...
    function apply(ctx) {
      ...
      ctx.slots.inject("conversation.view", function () {
        return ctx.slots.register({ name: "conversation.view", id: "dsh-sub-cli-jobs", order: 100, label: function () { return "CLI 任务"; } }, CliJobsView);
      });
    }
