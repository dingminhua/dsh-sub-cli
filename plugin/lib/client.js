    ...
    function SetupRow(props) {
      ...
      return React.createElement("section", { className: "dsc-card" },
        ...
        CLIS.map(function (cli) {
          ...
          var statusText = v
            ? (t("row.verified") + " " + (v.version || "-") + " · " + (v.at ? v.at.slice(0, 10) : "") + (v.provider && v.model ? " · " + v.provider + "/" + v.model : "") + (toolNote || ""))
            : (f ? (f.error || t("row.failed")) : t("row.notInstalled"));
