...
- 注册 **`cli_codex` / `cli_claude_code` / `cli_qwen`** 工具，让 DSH 模型把自包含任务交给对应 CLI；
...
- **Codex 持续会话**：首轮 `cli_codex` 返回稳定 `sessionId`；后续工具直接进入同一个 Codex thread，不经过 relay 模型；
...
5. 在对话中让 DSH 使用对应 CLI：日常委派使用 `cli_codex` / `cli_claude_code` / `cli_qwen`；仅明确要求一次性无头执行时使用 `cli_dispatch`。
